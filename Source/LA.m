...
if TT.openWtr
    ...
    fprintf(['Reading ' inFileNames.wtrFileName ' file'])
    TT.errCkWtr = true;
    ...
    oper = fopen(inFileNames.wtrFileName);
    if eq(oper,-1)
        error([inFileNames.wtrFileName ' file not found']);
    end
    fclose all;
    [wtrD,wtr,wtrHead] = gFileOpen(inFileNames.wtrFileName);
...
if TT.openWtr
    ...
    if isfield(inFileNames, 'salFileName')
        ...
        if eq(oper,-1)
            ...
            fclose all;
            [salD,sal,salHead] = gFileOpen(inFileNames.salFileName);
...
if TT.openWnd
    fprintf(['Reading ' inFileNames.wndFileName ' file'])
    TT.errCkWnd = true;
    ...
    oper = fopen(inFileNames.wndFileName);
    if eq(oper,-1)
...
if TT.openBth
    ...
    if eq(oper,-1)
        ...
        fclose all;
        bathArray = fileOpen(inFileNames.bthFileName,',',1);
    end
    bthD(1,:)   = bathArray(:,1);
    bthA(1,:)   = bathArray(:,2);
    clear bathArray
...
if TT.useSal
    ...
    if lt(thmMn,salMn)      % if the thermistors are closer to the surface
        fprintf(['Extending ' num2str(salDepths(1)) 'm'...
...
if TT.useLvl
    ...
    if any(isnan(lvl))
        rmvI = isnan(lvl);
        fprintf(['Removing ' datestr(lvlD(rmvI),'dd-mmm-yyyy HH:MM')...
...
if plotYes
    ...
    if writeTable.wTemp
        plotLA_results(writeTable,plotTable,dates,outFileNames,wtr,depthAry)
    else
        plotLA_results(writeTable,plotTable,dates,outFileNames)
    end
    fprintf('...completed\n\n');
end
...
if writeYes && ~isempty(writeNames)
    outFile = fopen(outFileNames.results,'w');
    if eq(outFile,-1)
...
if TT.wrt_wTemp && writeYes
    outFile = fopen(outFileNames.resultsWtr,'w');
    if eq(outFile,-1)
        error([outFileNames.resultsWtr ' file in use, please close']);
    end
